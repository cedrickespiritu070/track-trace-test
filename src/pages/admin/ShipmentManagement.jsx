...
const ShipmentManagement = () => {
  ...
  const downloadShipmentPDF = () => {
    if (selectedShipment) {
        ...
        pdf.setFontSize(16);
        pdf.text("Shipment Details", 10, logoY + logoHeight + 5);

        // Prepare data for the main shipment details table
        const tableData = [
        ...
        // Define columns for the main details table
        const columns = [
        ...
        // Add remarks as a table if available
        if (selectedShipment.remarks && selectedShipment.remarks.length > 0) {
            const remarksStartY = pdf.lastAutoTable.finalY + 10;
            pdf.setFontSize(12);
            pdf.text("Remarks:", 10, remarksStartY);

            // Prepare data for the remarks table
            const remarksTableData = selectedShipment.remarks.map((remark) => {
                let dateTime = "Not set";
                if (remark.date) {
                    dateTime = new Date(remark.date).toLocaleString("en-US", {
                        month: "long",
                        day: "numeric",
                        year: "numeric",
                        hour: "numeric",
                ...
                let additionalInfo = "";
                if (remark.status === "In Transit") {
                    additionalInfo = `Transit Type: ${remark.transitType || "Not set"}\nTransit Date: ${remark.transitDate && new Date(remark.transitDate).getTime() !== 0 ? new Date(remark.transitDate).toLocaleString("en-US", {
                        month: "long",
                        day: "numeric",
                        year: "numeric",
                        hour: "numeric",
                ...
                } else if (remark.status === "Departure at Origin") {
                    additionalInfo = `ATD: ${remark.atd ? new Date(remark.atd).toLocaleString("en-US", {
                        month: "long",
                        day: "numeric",
                        year: "numeric",
                        hour: "numeric",
                ...
                } else if (remark.status === "Arrival at Destination") {
                    additionalInfo = `AAD: ${remark.aad ? new Date(remark.aad).toLocaleString("en-US", {
                        month: "long",
                        day: "numeric",
                        year: "numeric",
                        hour: "numeric",
                ...
                return {
                    dateTime: dateTime,
                    status: remark.status || "No status available",
                    remarks: remark.remark || "No remarks available",
                    additionalInfo: additionalInfo,
                };
            });

            // Define columns for the remarks table
            const remarksColumns = [
                { header: "Date and Time", dataKey: "dateTime" },
                { header: "Status", dataKey: "status" },
                { header: "Remarks", dataKey: "remarks" },
                { header: "Additional Info", dataKey: "additionalInfo" },
            ];

            pdf.autoTable({
                startY: remarksStartY + 5,
                head: [remarksColumns.map(col => col.header)],
                body: remarksTableData.map(row => [row.dateTime, row.status, row.remarks, row.additionalInfo]),
                theme: "grid",
            });
        }
                ...
                <Tr key={index} className="hover:bg-gray-100">
                  <Td className="px-8 border-b border-gray-300 fixed-col bg-[#e5e7eb]">
                    <div className="flex flex-nowrap">
                      <button onClick={() => openModal(row)} className="bg-blue hover:bg-[#07335c] text-white px-[15px] py-[5px] text-[0.75rem] flex rounded-md gap-[0px] items-center">
                      <i className="fa-regular fa-eye"></i>View
                      </button>
                      {/* <button 
                        onClick={() => handleDelete(row)} 
            ...
            <div className="mt-4">
              <div className="flex flex-col">
                ...
                <div className="flex flex-row justify-between w-full mb-5">
                  ...
                  <span className="text-blue font-bold">{selectedShipment.status}</span>
                </div>
              </div>
