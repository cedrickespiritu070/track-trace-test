...
const ShipmentTable = ({ onRowClick, role, filterValue }) => {
  ...
  const editableShipmentFields = [
    ...
    'etd',
    'eta',
    // Add the SEA-specific fields
    'shipment_terms',
    'shipment_containers',
    'shipment_volume',
    'shipment_grossweight',
    // Add the AIR-specific fields
    'shipment_chargeableweight', // For AIR mode
    'shipment_grossweight', // For AIR mode
    'shipment_dimensions', // For AIR mode
  ];
  ...
  const handleFieldChange = (section, field, value) => {
    const fieldKey = field.toLowerCase(); // Simplified to match the column directly
  
    console.log(`Changing field: ${fieldKey}, New value: ${value}`);
    setSelectedShipment((prev) => {
      const updatedShipment = {
        ...prev,
        [fieldKey]: value,
      };
      console.log('Updated selectedShipment state:', updatedShipment);
      return updatedShipment;
    });
  };
  
  const handleSaveField = async (section, field, value) => {
    const fieldKey = field.toLowerCase(); // Ensure this matches your database column
  
    // Log for debugging purposes
    console.log(`Saving field: ${fieldKey}, Value: ${value}`);
    
    // Update the database with the corrected column name
...
const handleSendDocumentConfirmation = (doc) => {
  confirmAlert({
    customUI: ({ onClose }) => (
      <div className="bg-white p-6 rounded-lg shadow-lg relative">
        ...
        <h2 className="text-lg font-semibold text-blue-500">Confirm Send Document</h2>
        <p className="mt-2">
          Are you sure you want to send the document titled: {doc.document_title}?
        </p>
        <div className="mt-4">
          <div className="flex justify-between mt-6">
            ...
            <button
              onClick={() => {
                handleSendDocument(doc); // Call actual send function if confirmed
...
// Existing handleSendDocument function
const handleSendDocument = async (doc) => {
  try {
    // Step 1: Get the current value of the document_urls column
    const { data: currentShipment, error: fetchError } = await supabase
      .from('shipments')
      .select('document_urls')
      .eq('shipment_id', selectedShipmentForView.shipment_id)
      .single();

    if (fetchError) {
      console.error('Error fetching current document URLs:', fetchError);
      throw new Error('Error fetching current document URLs');
    }

    // Step 2: Parse the current document_urls if necessary
    let updatedDocumentUrls = currentShipment.document_urls || [];
    if (typeof updatedDocumentUrls === 'string') {
      updatedDocumentUrls = JSON.parse(updatedDocumentUrls);
    }

    // Step 3: Append the new document object (title and URL) to the existing array
    updatedDocumentUrls.push({
      title: doc.document_title,
      url: doc.document_url,
    });

    // Step 4: Update the shipments table with the new array
    ...
    if (shipmentError) {
      ...
      throw new Error('Error updating shipments table: ' + shipmentError.message);
    }

    // Step 5: Show success notification
    toast.success('Document sent and shipment updated successfully!');
  } catch (error) {
  ...
  // The function triggered when the "Save Status" button is clicked
  const handleStatusChange = () => {
    if (selectedShipment) {
      ...
      const transitDateValue = transitDate || "N/A"; // Already in ISO format from the input
  ...
  const saveShipmentData = async (
    ...
    const statusToStep = {
      "Order Placed": 1,
      "In Warehouse": 2,
      "Departure at Origin": 3,
      "In Transit": 4,
      "Arrival at Destination": 5,
      "Custom Clearance": 6,
      Delivered: 7,
    };
    ...
    // Get the last remark if it exists
    const lastRemark = existingRemarks[existingRemarks.length - 1] || {};
    ...
    // Set transitDate to null if it's invalid or not provided
    const resolvedTransitDate = isValidDate(transitDate)
      ? new Date(transitDate).toISOString()
      : null;
  
    // Ensure atd and aad are not overwritten with null if already set
    const newAtd = actualDepartureDate || (lastRemark.atd ? lastRemark.atd : null);
    const newAad = actualArrivalDate || (lastRemark.aad ? lastRemark.aad : null);
    ...
    const newRemarkObject = {
      ...
      transitDate: resolvedTransitDate, // Will be null if invalid or not provided
      atd: newAtd,
      aad: newAad,
    };
    ...
    <div className="shipment-table-container">
            ...
            <Table className="min-w-full table-auto border-collapse border border-gray-300">
              ...
              <Tbody>
                {filteredData.map((shipment, index) => (
                  ...
                  <Td className="px-8 border-b border-gray-300 truncate">
                        {
                          // Get the most recent ATD (Actual Departure Date) from remarks if available
                          shipment.remarks?.length > 0 &&
                          shipment.remarks[shipment.remarks.length - 1].atd
                            ? new Intl.DateTimeFormat('en-US', {
                                ...
                                minute: '2-digit',
                                hour12: true, // AM/PM format
                              }).format(new Date(shipment.remarks[shipment.remarks.length - 1].atd))
                            : "-"
                        }
                      </Td>
                      ...
                      <Td className="px-8 border-b border-gray-300 truncate">
                        {
                          // Get the most recent AAD (Actual Arrival Date) from remarks if available
                          shipment.remarks?.length > 0 &&
                          shipment.remarks[shipment.remarks.length - 1].aad
                            ? new Intl.DateTimeFormat('en-US', {
                                ...
                                minute: '2-digit',
                                hour12: true, // AM/PM format
                              }).format(new Date(shipment.remarks[shipment.remarks.length - 1].aad))
                            : "-"
                        }
                      </Td>
      ...
      {selectedShipment && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white w-[90%] max-w-[40%] max-h-[65rem] h-[80vh] p-6 rounded-lg shadow-lg relative flex flex-col justify-between">
            <button
              onClick={closeModal}
              className="absolute top-2 right-2 text-gray-500 text-[30px] hover:text-gray-700"
            >
            ...
            {activeTab === 'shipment' && (
                <div className="flex flex-col">
                  ...
                  {[
                    { label: 'Description', key: 'shipment_description' },
                    { label: 'Type', key: 'shipment_type' },
                    { label: 'Instructions', key: 'shipment_instructions' },
                    { label: 'Mode', key: 'shipment_mode' },
                    {
                      label: 'Gross Weight',
                      key: 'shipment_grossweight',
                      disabled: selectedShipment.shipment_mode === 'SEA'
                    },
                    {
                      label: 'Chargeable Weight',
                      key: 'shipment_chargeableweight',
                      disabled: selectedShipment.shipment_mode === 'SEA'
                    },
                    {
                      label: 'Dimensions',
                  ...
                  ].map(({ label, key, type, options, isDate, disabled }) => (
                    <div className="flex flex-row justify-between w-full mb-5" key={key}>
                      <span className="text-gray-500">Shipment {label}</span>
                      {type === 'dropdown' ? (
                        <select
                          value={selectedShipment[key] || ''}
                          onChange={(e) => {
                            const newValue = e.target.value;
                            handleFieldChange('shipment', key, newValue);
                            handleSaveField('shipment', key, newValue);
                          }}
                          className="border border-gray-300 p-1 rounded text-blue"
                          disabled={disabled}
                        >
                          ...
                          value={
                            isDate
                              ? selectedShipment[key]
                                ? new Date(selectedShipment[key]).toISOString().split('T')[0]
                                : ''
                              : selectedShipment[key] || ''
                          }
                          onChange={(e) => {
                            ...
                            handleFieldChange('shipment', key, newValue);
                            handleSaveField('shipment', key, newValue); // Ensure this function saves data to your database
                          }}
                          className="border border-gray-300 p-1 rounded text-blue"
                          disabled={disabled}
                        />
                  ...
                  {/* Conditional fields for non-AIR mode */}
                  {selectedShipment.shipment_mode !== 'AIR' && (
                    <>
                      <div className="flex flex-row justify-between w-full mb-5">
                        <span className="text-gray-500">Shipment Terms</span>
                        <select
                          value={selectedShipment.shipment_terms || ''}
                          onChange={(e) => {
                            const newValue = e.target.value;
                            handleFieldChange('shipment', 'shipment_terms', newValue);
                            handleSaveField('shipment', 'shipment_terms', newValue);
                          }}
                          className="border border-gray-300 p-1 rounded text-blue"
                        >
                      ...
                      {selectedShipment.shipment_terms === 'FCL' && (
                        <>
                          <div className="flex flex-row justify-between w-full mb-5">
                            <span className="text-gray-500">Containers</span>
                            <input
                              type="text"
                              value={selectedShipment.shipment_containers || ''}
                              onChange={(e) => {
                                const newValue = e.target.value;
                                handleFieldChange('shipment', 'shipment_containers', newValue);
                                handleSaveField('shipment', 'shipment_containers', newValue);
                              }}
                              className="border border-gray-300 p-1 rounded text-blue"
                            />
                          </div>
                          <div className="flex flex-row justify-between w-full mb-5">
                            ...
                            <input
                              type="text"
                              value={selectedShipment.shipment_weight || ''}
                              onChange={(e) => {
                                const newValue = e.target.value;
                                handleFieldChange('shipment', 'shipment_weight', newValue);
                                handleSaveField('shipment', 'shipment_weight', newValue);
                              }}
                              className="border border-gray-300 p-1 rounded text-blue"
                            />
                          </div>
                      ...
                      {selectedShipment.shipment_terms === 'LCL' && (
                        <>
                          <div className="flex flex-row justify-between w-full mb-5">
                            <span className="text-gray-500">Packages</span>
                            <input
                              type="text"
                              value={selectedShipment.shipment_packages || ''}
                              onChange={(e) => {
                                const newValue = e.target.value;
                                handleFieldChange('shipment', 'shipment_packages', newValue);
                                handleSaveField('shipment', 'shipment_packages', newValue);
                              }}
                              className="border border-gray-300 p-1 rounded text-blue"
                            />
                          </div>
                          <div className="flex flex-row justify-between w-full mb-5">
                            ...
                            <input
                              type="text"
                              value={selectedShipment.shipment_volume || ''}
                              onChange={(e) => {
                                const newValue = e.target.value;
                                handleFieldChange('shipment', 'shipment_volume', newValue);
                                handleSaveField('shipment', 'shipment_volume', newValue);
                              }}
                              className="border border-gray-300 p-1 rounded text-blue"
                            />
                          </div>
                          <div className="flex flex-row justify-between w-full mb-5">
                            ...
                            <input
                              type="text"
                              value={selectedShipment.shipment_grossweight || ''}
                              onChange={(e) => {
                                const newValue = e.target.value;
                                handleFieldChange('shipment', 'shipment_grossweight', newValue);
                                handleSaveField('shipment', 'shipment_grossweight', newValue);
                              }}
                              className="border border-gray-300 p-1 rounded text-blue"
                            />
                          </div>
              ...
              <div>
                ...
                {editableStatus === "Departure at Origin" && (
                  <div className="flex flex-row justify-between w-full mb-5">
                    <span className="text-gray-500">Actual Departure Date and Time</span>
                    <input
                      type="datetime-local"
                ...
                {editableStatus === "Arrival at Destination" && (
                  <div className="flex flex-row justify-between w-full mb-5">
                    <span className="text-gray-500">Actual Arrival Date and Time</span>
                    <input
                      type="datetime-local"
    ...
    {currentDocuments.map((doc) => (
      <div key={doc.id} className="document-item mb-2">
        <p className="text-md text-blue font-medium">
          <strong>Title:</strong> {doc.document_title || "No Title Provided"}
        </p>
        <p className="text-md text-blue font-medium">
          <strong>Document Name:</strong> {doc.document_name || "No Document Name"}
        </p>
        <a
        ...
        <button
          onClick={() => handleSendDocumentConfirmation(doc)} // Confirm before sending
          className="bg-blue text-white px-4 mx-2 rounded"
        >
          Send
        </button>
      </div>
